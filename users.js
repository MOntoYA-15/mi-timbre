...
function pintarUsuarios() {
    ...
    const admins = usuariosCache.filter(u => u.rol === 'admin').length;
    const activos = usuariosCache.filter(u => u.activo !== false).length;
    const resumen = document.getElementById('resumen-usuarios');
    if (resumen) resumen.innerHTML = `<span>👑 Admins: <b>${admins}</b></span><span>✅ Activos: <b>${activos}</b></span><span>⛔ Inactivos: <b>${usuariosCache.length - activos}</b></span>`;
        ...
        div.innerHTML = `
            <div class="usuario-top">
                <span class="usuario-email">
                    ...
                    <span class="estado-pill ${activo ? 'on' : 'off'}">${activo ? 'Activo' : 'Inactivo'}</span>
                </span>
